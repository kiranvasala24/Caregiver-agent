...
export async function GET(req: NextRequest) {
  ...
  console.log("Connect callback hit, connect_code:", connectCode);

  if (!connectCode) {
    return NextResponse.redirect(
      new URL("/connect?error=no_code", process.env.APP_BASE_URL!)
    );
  }
  ...
  try {
    ...
    if (!tokenResult?.token) {
      return NextResponse.redirect(
        new URL("/connect?error=no_token", process.env.APP_BASE_URL!)
      );
    }
    ...
    if (!res.ok) {
      ...
      return NextResponse.redirect(
        new URL("/connect?error=complete_failed", process.env.APP_BASE_URL!)
      );
    }
    ...
    return NextResponse.redirect(
      new URL("/connect?error=exception", process.env.APP_BASE_URL!)
    );
  }
}